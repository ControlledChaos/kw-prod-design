...
function use_meta_tags() {
	return apply_filters( 'kwpd_use_meta_tags', true );
}
...
function use_structured_data() {
	return apply_filters( 'kwpd_use_structured_data', true );
}
...
function title() {
	...
	return apply_filters( 'kwpd_meta_data_title', $title );
}
...
function subtitle() {
	$subtitle = '';
	return apply_filters( 'kwpd_meta_data_subtitle', $subtitle );
}
...
function description() {
	...
	$search = wp_strip_all_tags(
		sprintf(
			'%s %s',
			__( 'Showing results for', 'kw-prod-design' ),
			get_search_query()
		)
	);
	...
	if ( ! empty( $site_description ) && ( is_front_page() || is_home() ) ) {
		...
		$description = __( '404 Error: Not Found', 'kw-prod-design' );

	} else {
		$description = '';
	}

	return apply_filters( 'kwpd_meta_data_description', $description );
}
...
function url() {
	...
	return apply_filters( 'kwpd_meta_data_url', $url );
}
...
function published() {
	...
	return apply_filters( 'kwpd_meta_data_published', $date );
...
function modified() {
	...
	return apply_filters( 'kwpd_meta_data_modified', $date );
}
...
function post_genre() {
	$genre = '';
	return apply_filters( 'kwpd_meta_data_post_genre', $genre );
}
...
function get_keywords() {
	return apply_filters( 'kwpd_meta_data_get_keywords', [] );
}
...
function keywords() {
	...
	return apply_filters( 'kwpd_meta_data_keywords', $keywords );
}
...
function image() {
	...
	return apply_filters( 'kwpd_meta_data_image', $src );
}
...
function site_logo( $html = null ) {
	...
	return apply_filters( 'kwpd_meta_data_logo', $logo );
}
...
function copyright() {
	...
	return apply_filters( 'kwpd_meta_data_copyright', $copy );
}
...
function print_meta_tags() {
	...
	return apply_filters( 'kwpd_print_meta_tags', $tags );
}
...
function print_structured_data() {
	...
	if ( is_singular( 'post' ) && 'news' === KWPD_POSTS_CONTENT_TYPE ) {
		$data = news_article_data();
	} elseif ( is_singular( 'post' ) && 'blog' === KWPD_POSTS_CONTENT_TYPE ) {
		$data = blog_post_data();
	} elseif ( is_singular() ) {
		$data = article_data();
	}
	return apply_filters( 'kwpd_print_structured_data', $data );
}
 ...
 */
function meta_tags() {
	include KWPD_PATH . 'views/frontend/meta-data/meta-tags/standard.php';
}
...
function schema_tags() {
	include KWPD_PATH . 'views/frontend/meta-data/meta-tags/schema.php';
}
...
function open_graph_tags() {
	include KWPD_PATH . 'views/frontend/meta-data/meta-tags/open-graph.php';
}
...
function twitter_tags() {
	include KWPD_PATH . 'views/frontend/meta-data/meta-tags/twitter.php';
}
...
function dublin_tags() {
	include KWPD_PATH . 'views/frontend/meta-data/meta-tags/dublin.php';
}
...
function article_data() {
	include KWPD_PATH . 'views/frontend/meta-data/structured-data/article.php';
}
...
function news_article_data() {
	include KWPD_PATH . 'views/frontend/meta-data/structured-data/news-article.php';
}
...
function blog_post_data() {
	include KWPD_PATH . 'views/frontend/meta-data/structured-data/blog-post.php';
}
