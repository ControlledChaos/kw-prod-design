...
function get_row_notice() {
	add_action( 'after_plugin_row_' . KWPD_BASENAME, __NAMESPACE__ . '\row_notice', 5, 3 );
}
...
function row_notice( $plugin_file, $plugin_data, $status ) {
	...
	?>
	<style>
		.plugins tr[data-plugin='<?php echo KWPD_BASENAME; ?>'] th,
		.plugins tr[data-plugin='<?php echo KWPD_BASENAME; ?>'] td {
			box-shadow: none;
		}

		<?php if ( isset( $plugin_data['update'] ) && ! empty( $plugin_data['update'] ) ) : ?>

			.plugins tr.<?php echo 'kw-prod-design'; ?>-plugin-tr td {
				box-shadow: none ! important;
			}

			.plugins tr.<?php echo 'kw-prod-design'; ?>-plugin-tr .update-message {
				margin-bottom: 0;
			}
	...
	<tr id="plugin-php-notice" class="plugin-update-tr active <?php echo 'kw-prod-design'; ?>-plugin-tr">
		<td colspan="<?php echo $colspan; ?>" class="plugin-update colspanchange">
			<div class="update-message notice inline notice-error notice-alt">
				<?php echo sprintf(
					'<p>%s %s %s %s %s %s</p>',
					__( 'Functionality of the', 'kw-prod-design' ),
					KWPD_NAME,
					__( 'plugin has been disabled because it requires PHP version', 'kw-prod-design' ),
					KWPD_MIN_PHP_VERSION,
					__( 'or greater. Your system is running PHP version', 'kw-prod-design' ),
					phpversion()
				); ?>
			</div>
		</td>
